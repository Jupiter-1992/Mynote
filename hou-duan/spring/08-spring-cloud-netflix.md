...
1. 在 Application 中增加 `@EnableHystrix` 注解：
...
1. 在 Service 中增加 `@HystrixCommand` 注解：
...
启动后访问仪表盘：[http://localhost:8764/hystrix。](http://localhost:8764/hystrix。)
...
| BAD\_REQUEST | 执行抛出 HystrixBadRequestException | NO |
...
![Hystrix\_Dashboard\_&#x754C;&#x9762;&#x76D1;&#x63A7;&#x53C2;&#x6570;](https://raw.githubusercontent.com/chanshiyucx/yoi/master/2019/Spring-Cloud-Netflix/Hystrix_Dashboard_界面监控参数.png)
...
打开浏览器访问：[http://localhost:8769/api/a/say?message=HelloZuul](http://localhost:8769/api/a/say?message=HelloZuul) 浏览器显示：
...
启动后访问 [http://localhost:8769/api/a/say?message=HelloZuul，显示：](http://localhost:8769/api/a/say?message=HelloZuul，显示：)

```text
Token is empty
```

带上 token 后访问 [http://localhost:8769/api/b/say?message=HelloZuul&token=123，显示：](http://localhost:8769/api/b/say?message=HelloZuul&token=123，显示：)
